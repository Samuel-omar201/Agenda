...
function listContactToUserLogged( req, res ){
    let idUser = req.params.idUser

    Usuario.findById( idUser, (err, result)=>{
        if( err){
            return res.send({ message: `error is ${err} ` })
        }
...
function saveUser(req, res) {
    ...
    if (params.primer_nombre && params.celular && params.correo && params.password) {
...
function login(req, res) {
    ...
    if (correo, password) {
        Usuario.findOne({ correo: correo, password: password }, (err, user) => {
            if (err) {
